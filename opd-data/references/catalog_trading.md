...
| `sec_code` | 否 | 证券代码；操作符：in（逗号拆分多值（如 2024-01-01,2024-01-02）） | string |  |
| `market_code` | 否 | 交易市场编码；枚举值按「value - label」展示（value 为实际传参值，label 为含义），调用时仅传 value 部分 | string | 012001 - 上交所；012002 - 深交所主板；012015 - 深交所创业板；012029 - 上交所科创板；012046 - 北交所 |
| `sec_category_code` | 否 | 证券类别编码；枚举值按「value - label」展示（value 为实际传参值，label 为含义），调用时仅传 value 部分 | string | 001001 - A股；001002 - B股；001004 - 股份报价；001013 - CDR |
...
| `sec_code` | 否 | 证券代码；操作符：in（逗号拆分多值（如 2024-01-01,2024-01-02）） | string |  |
| `market_code` | 否 | 交易市场编码；枚举值按「value - label」展示（value 为实际传参值，label 为含义），调用时仅传 value 部分 | string | 012001 - 上交所；012002 - 深交所主板；012015 - 深交所创业板；012029 - 上交所科创板；012046 - 北交所 |
| `sec_category_code` | 否 | 证券类别编码；枚举值按「value - label」展示（value 为实际传参值，label 为含义），调用时仅传 value 部分 | string | 001001 - A股；001002 - B股；001013 - CDR；002001 - 国债；002028 - 私募债；003001 - 封闭式基金；003002 - 开放式基金；003008 - QDII |
...
| 参数 | 必填 | 说明 | 类型 | 示例 / 可选值 |
|---|---|---|---|---|
| `exchange_code` | 否 | 交易所编码；枚举值按「value - label」展示（value 为实际传参值，label 为含义），调用时仅传 value 部分 | string | 012001 - 上交所；012002 - 深交所主板；012006 - 港交所主板；012007 - 港交所创业板；012008 - 股份报价系统 |
